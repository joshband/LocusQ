...
2. **Ephemeral emitter audio block pointer as v1 fast path** for renderer-side spatialization in the same process block.
...
- Ephemeral pointer lifetime requires strict invariants and validation discipline.

## Guardrails

1. Pointer validity is limited to the current `processBlock` cycle.
