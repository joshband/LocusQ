...
| SceneGraph pooled emitter-audio reservation contract | `Source/SceneGraph.h` + `Source/PluginProcessor.h` + `Source/PluginProcessor.cpp` + `Source/processor_core/ProcessorSceneRegistration.cpp` | W2-A completes MQ-2 by moving per-slot mono audio scratch out of the fixed `EmitterSlot` footprint and into preclaimed prepare-sized reservations that bind during lock-free emitter registration, preserving stable slot identity while avoiding heap allocation in `processBlock` | `TestEvidence/build-summary.md` |
...
| `bake_kf_density` | `emitter_choreography` | `ProcessorParameterLayout.cpp`; `handleAsyncUpdate` → `BakeRecorder::prepare()` | Pending BL-116 | Float 0.5–100.0 kf/s. Seeds target spacing; adaptive refinement may add keyframes to preserve the raw-sample error bound. |
| `bake_curve_fit_tolerance` | `emitter_choreography` | `ProcessorParameterLayout.cpp`; `handleAsyncUpdate` → `BakeRecorder::prepare()` | Pending BL-116 | Float 0.001–1.0 m. Maximum linear-interpolation error checked against every captured sample. |

Registration source: `Source/processor_core/ProcessorParameterLayout.cpp` choreography group (CL-P7 block). BakeRecorder owned by `ChoreographyWorker`; `tick()` called in `compute()` on PhysicsWorker thread (no allocation, captures to pre-allocated buffer). `prepare()` and `exportToTimeline()` called from main thread in `handleAsyncUpdate()`. Export writes `bake_pos_x_N`, `bake_pos_y_N`, `bake_pos_z_N` scalar tracks per active emitter into `keyframeTimelineState`, then pushes to RT triple-buffer via `publishKeyframeTimelineStateToRtLocked()`. The deterministic lifecycle/export contract is exercised by `locusq_bake_recorder_probe` and emits `qa_output/locusq_bake_recorder_probe_result.json`.
