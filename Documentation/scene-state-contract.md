...
- Top-level fields:
  ...
  - Headphone render diagnostics:
    ...
    - Head-tracking bridge diagnostics (additive):
      ...
      - `rendererHeadTrackingConsumers` (uint32; count of LocusQ instances in current host process sharing the bridge receiver)
      ...
      - `rendererHeadTracking` (object mirror with `enabled`, `source`, `poseAvailable`, `poseStale`, `orientationValid`, `invalidPackets`, `consumers`, `seq`, `timestampMs`, `ageMs`, `qx`, `qy`, `qz`, `qw`, `yawDeg`, `pitchDeg`, `rollDeg`)
