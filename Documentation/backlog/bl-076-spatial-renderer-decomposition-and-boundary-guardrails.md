...
BL-076 in plain terms: Decompose Source/SpatialRenderer.h into cohesive renderer modules with explicit ownership boundaries so the runtime can evolve without a single giant multipurpose header becoming a merge-risk and defect hotspot. Current state: In Implementation (Wave 6 audition parity lifecycle helper extraction landed; contract+execute replay PASS on 2026-03-04). For technical detail, see `## Objective` and `## Validation Plan`.
...
| When is it done? | Current state: In Implementation (Wave 6 audition parity lifecycle helper extraction landed; contract+execute replay PASS on 2026-03-04). This item is done when required acceptance checks pass and promotion evidence is complete. |
...
| ID | BL-076 |
| Priority | P1 |
| Status | In Implementation (Wave 6 audition parity lifecycle helper extraction landed; contract+execute replay PASS on 2026-03-04) |
| Track | F - Hardening |
| Effort | High / L |
...
- Evidence roots:
  - `TestEvidence/bl076_spatial_renderer_20260304T193739Z/` (contract-only)
  - `TestEvidence/bl076_spatial_renderer_20260304T193750Z/` (execute)
