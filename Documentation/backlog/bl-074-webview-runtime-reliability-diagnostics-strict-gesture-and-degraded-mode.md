...
BL-074 focuses on a clear, operator-visible outcome: Improve WebView runtime trust by making gesture-path failures explicit in self-test CI (strict_gesture mode), surfacing native-call binding failures in an operator-visible diagnostics channel, and introducing a deterministic degraded mode when critical startup bindings fail. This matters because it improves reliability and decision confidence for nearby release lanes. Current state: Open.
