...
BL-034 in plain terms: Define and enforce a deterministic verification + profile-governance layer for headphone calibration so profile selection, fallback behavior, and perceptual verification outcomes are reproducible, machine-readable, and release-governance ready. Current state: Done (Owner Z6 promotion finalized; Owner Z7 post-done confidence replay remains green). For technical detail, see `## Objective` and `## Validation Plan`.
