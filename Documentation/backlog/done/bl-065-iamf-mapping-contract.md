...
BL-065 in plain terms: Define deterministic mapping contract from ambisonics IR into IAMF scene/profile outputs, including transform rules, profile constraints, parity evidence schema, and blocker routing. Current state: Done. For technical detail, see `## Objective` and `## Validation Plan`.
