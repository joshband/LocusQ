...
This runbook tracks **BL-027** (BL-027 — Renderer UX v2). Current status: **Done (Promoted via Slice F packet)**. In plain terms: This runbook defines a scoped change with explicit validation and evidence requirements.
...
| What is changing? | BL-027 — Renderer UX v2 |
...
| Where is the source of truth? | Runbook: `Documentation/backlog/done/bl-027-renderer-uiux-v2.md` plus repo-local evidence under `TestEvidence/...`. |
...
|---|---|
| ID | BL-027 |
| Status | Done (Promoted via Slice F packet) |
| Priority | P2 |
...
### Slice C Execution Snapshot (2026-02-25)

1. Implemented a collapsible Steam diagnostics card in the renderer panel, collapsed by default, with additive field rows and no bridge/schema changes.
2. Wired deterministic per-field fallback resolution for missing/partial payloads in `updateRendererPanelShell`:
   - HRTF status
   - Binaural quality tier
   - Reflections state
   - Convolution method
   - Last error (row hidden when empty)
3. Validation status (`TestEvidence/bl027_slice_c_20260225T180311Z/status.tsv`):
   - `node --check Source/ui/public/js/index.js` => PASS
   - `cmake --build build_local --config Release --target LocusQ_Standalone -j 8` => PASS
   - `LOCUSQ_UI_SELFTEST_SCOPE=bl029 ./scripts/standalone-ui-selftest-production-p0-mac.sh` => PASS x3
   - `./scripts/validate-docs-freshness.sh` => PASS
4. Evidence bundle: `TestEvidence/bl027_slice_c_20260225T180311Z/`.
5. Disposition: Slice C accepted with no blockers.

### Slice D Execution Snapshot (2026-02-25)

1. Implemented a collapsible Ambisonic diagnostics card in the renderer panel, collapsed by default, with additive field rows and no bridge/schema changes.
2. Wired deterministic per-field fallback resolution for missing/partial payloads in `updateRendererPanelShell`:
   - Order
   - Channel count
   - Decoder state
   - Decoder type
3. Validation status (`TestEvidence/bl027_slice_d_20260225T181011Z/status.tsv`):
   - `node --check Source/ui/public/js/index.js` => PASS
   - `cmake --build build_local --config Release --target LocusQ_Standalone -j 8` => PASS
   - `LOCUSQ_UI_SELFTEST_SCOPE=bl029 ./scripts/standalone-ui-selftest-production-p0-mac.sh` => PASS x3
   - `./scripts/validate-docs-freshness.sh` => PASS
4. Evidence bundle: `TestEvidence/bl027_slice_d_20260225T181011Z/`.
5. Disposition: Slice D accepted with no blockers.

### Slice E Execution Snapshot (2026-02-25)

1. Finalized cross-panel coherence by making profile updates deterministic across CALIBRATE and RENDERER:
   - single-shot scene+calibration JS dispatch per timer tick (`Source/PluginEditor.cpp`)
   - additive `profileSyncSeq` publication in scene snapshots and calibration status (`Source/PluginProcessor.cpp`)
   - monotonic stale-state rejection in WebView update paths (`Source/ui/public/js/index.js`)
2. Added scene-authoritative fallback in calibration chip/rendering path so rapid bursts cannot leave lagging panel labels when calibration status trails scene payloads.
3. Validation status (`TestEvidence/bl027_slice_e_20260225T182057Z/status.tsv`):
   - `node --check Source/ui/public/js/index.js` => PASS
   - `cmake --build build_local --config Release --target LocusQ_Standalone -j 8` => PASS
   - `LOCUSQ_UI_SELFTEST_SCOPE=bl029 ./scripts/standalone-ui-selftest-production-p0-mac.sh` => PASS x5
   - `./scripts/validate-docs-freshness.sh` => PASS
4. Evidence bundle: `TestEvidence/bl027_slice_e_20260225T182057Z/`.
5. Disposition: Slice E accepted with no blockers.

### Slice F Done Promotion Packet Snapshot (2026-02-25)

1. Executed promotion validation gates on current branch state:
   - `node --check Source/ui/public/js/index.js`
   - `cmake --build build_local --config Release --target LocusQ_Standalone -j 8`
   - `LOCUSQ_UI_SELFTEST_SCOPE=bl029 ./scripts/standalone-ui-selftest-production-p0-mac.sh` x5
   - `./scripts/validate-docs-freshness.sh`
2. Compiled explicit Slice A-E evidence matrix with authoritative status paths:
   - Slice A: `TestEvidence/owner_bl027_slice_a_replay_20260225T175000Z/status.tsv`
   - Slice B: `TestEvidence/bl027_slice_b_impl_20260225T175738Z/status.tsv`
   - Slice C: `TestEvidence/bl027_slice_c_20260225T180311Z/status.tsv`
   - Slice D: `TestEvidence/bl027_slice_d_20260225T181011Z/status.tsv`
   - Slice E: `TestEvidence/bl027_slice_e_20260225T182057Z/status.tsv`
3. Promotion packet evidence bundle: `TestEvidence/bl027_done_promotion_slice_f_20260225T205629Z/`.
4. Promotion decision: `GO` — BL-027 is promotion-ready (`Slices A-E represented and PASS`).
...
| Archived Runbook Path | `Documentation/backlog/done/bl-027-renderer-uiux-v2.md` |
