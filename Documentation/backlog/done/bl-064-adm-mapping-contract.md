...
BL-064 in plain terms: Define deterministic mapping contract from ambisonics IR into ADM-targeted metadata/audio representation with explicit transform invariants, parity schema, and fallback behavior. Current state: Done. For technical detail, see `## Objective` and `## Validation Plan`.
