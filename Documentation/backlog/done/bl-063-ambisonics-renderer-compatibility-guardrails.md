...
BL-063 in plain terms: Define deterministic guardrails and parity checks that ensure ambisonics IR integration preserves renderer compatibility across supported output layouts before pilot intake. Current state: Done. For technical detail, see `## Objective` and `## Validation Plan`.
