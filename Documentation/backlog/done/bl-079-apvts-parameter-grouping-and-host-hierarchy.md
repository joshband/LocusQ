...
Last Modified Date: 2026-03-19 (Done — host-view gate PASS; all 20 group-boundary names confirmed at expected flat indices in VST3 host)

# BL-079: APVTS Parameter Grouping and Host Hierarchy

## Plain-Language Summary

BL-079 in plain terms: organize LocusQ's parameter list into sensible host-visible groups so DAWs can show cleaner sections like Calibration, Emitter, and Renderer without breaking existing sessions, automation IDs, or the WebView bridge. Current state: Done. The grouped APVTS tree is implemented locally, the clean-checkout replay passes, and the REAPER host gate now passes for both VST3 and AU with the correct format-aware flat-order expectation.
...
| ID | BL-079 |
| Priority | P2 |
| Status | Done (2026-03-19: host-view gate PASS — 215 params, 20/20 group-boundary names confirmed, Mode at idx 0, Renderer immediately after Emitter) |
...
| ~~Host smoke follow-up~~ | `[DONE]` | P2 | Small | VST3 gate PASS 2026-03-19: 215 params, all 20 group-boundary names confirmed, Mode at idx 0, Renderer immediately follows Emitter. AU gate: Mode at idx 1 — expected JUCE AU bypass-param convention (framework inserts implicit bypass at idx 0), not a layout defect. | `n/a` | 2026-03-19 | `TestEvidence/bl079_param_group_host_gate_vst3_20260319T042416Z/` | none |
...
- `scripts/reaper-param-group-host-gate-mac.sh --format AU` -> `PASS`
- AU host gate: `param_count=214`, all 20 required names present, `Mode` at index `1` (expected — JUCE AU implicit bypass at idx 0), renderer section ordered after emitter identity
- evidence root: `TestEvidence/bl079_param_group_host_gate_au_20260319T042752Z`
- disposition: shared RT-safety blocker is cleared; REAPER VST3 and AU host gates are both green, so BL-079 is closed
...
| Host-specific parameter-tree presentation differs across formats | Medium | Medium | Keep format-aware host expectations explicit in the host-gate evidence and rerun before closeout if JUCE/AU plumbing changes |
| Future host versions reshuffle AU flat ordering again | Medium | Low | Re-run the REAPER AU host gate when the JUCE/AU wrapper changes |
...
- [x] Representative AU host parameter-view gate reconciled (Mode at idx 1 in AU is the JUCE implicit bypass-param convention, not a layout defect; VST3 is the authoritative host-view proof)
- [x] Promotion decision recorded (VST3 gate PASS 2026-03-19T042416Z is the promotion evidence)
