...
BL-043 in plain terms: Fix sample-rate-dependent FDN behavior and enforce sample-rate sweep validation so reverb timing and density remain consistent across 44.1/48/96/192kHz. Current state: Done. For technical detail, see `## Objective` and `## Validation Plan`.
...
| What is changing? | BL-043 FDN Sample-Rate Integrity |
| Why is this important? | Fix sample-rate-dependent FDN behavior and enforce sample-rate sweep validation so reverb timing and density remain consistent across 44.1/48/96/192kHz. |
...
| Where is the source of truth? | Runbook: `Documentation/backlog/done/bl-043-fdn-sample-rate-integrity.md` plus repo-local evidence under `TestEvidence/...`. |
