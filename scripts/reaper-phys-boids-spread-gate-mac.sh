#!/usr/bin/env bash
# LocusQ boids-spread host gate wrapper for headless REAPER.
#
# Opens a preloaded REAPER project with LocusQ on Track 1, runs the boids-
...
GATE_SCRIPT="$ROOT_DIR/qa/reaper/reascripts/LocusQ_PhysicsBoidsSpread_Gate.lua"
...
RUN_DIR="$ROOT_DIR/TestEvidence/reaper_phys_boids_spread_gate_${TIMESTAMP}"
mkdir -p "$RUN_DIR"
...
if [[ "$GATE_OVERALL" == "pass" ]]; then
  echo "PASS: Physics boids-spread host gate"
  exit 0
fi

echo "FAIL: Physics boids-spread host gate — see $GATE_STATUS" >&2
exit 1
