...
DEFAULT_OUT_DIR="$ROOT_DIR/TestEvidence/bl036_finite_output_lane_${TIMESTAMP}"
OUT_DIR="${BL036_OUT_DIR:-$DEFAULT_OUT_DIR}"
SCENARIO_PATH="${BL036_SCENARIO_PATH:-$ROOT_DIR/qa/scenarios/locusq_bl036_finite_output_suite.json}"
QA_BIN="${BL036_QA_BIN:-$ROOT_DIR/build_local/locusq_qa_artefacts/Release/locusq_qa}"
if [[ ! -x "$QA_BIN" ]]; then
  QA_BIN="${BL036_QA_BIN_FALLBACK:-$ROOT_DIR/build_local/locusq_qa_artefacts/locusq_qa}"
fi

SCRIPT_PATH="$ROOT_DIR/scripts/qa-bl036-finite-output-lane-mac.sh"

RUN_MODE="contract_only"
RUNS="${BL036_RUNS:-1}"
SKIP_BUILD="${BL036_SKIP_BUILD:-0}"

usage() {
  cat <<USAGE
Usage: ./scripts/qa-bl036-finite-output-lane-mac.sh [options]

Options:
  --out-dir <path>     Artifact output directory (overrides BL036_OUT_DIR).
  --scenario <path>    Scenario suite path (overrides BL036_SCENARIO_PATH).
  --qa-bin <path>      QA runner path (overrides BL036_QA_BIN).
  --runs <N>           Deterministic run count, integer >= 1 (default: 1).
  --contract-only      Validate BL-036 contract lane only (default).
...
if [[ "$RUNS" -gt 1 ]]; then
  ...
  printf "run_id\trun_dir\texit_code\tlane_result\tstatus_fail_count\tcontract_schema\tboundary_contract\tcontainment_contract\tscenario_status\tcombined_signature\tsignature_match\trow_signature\tbaseline_row_signature\trow_match\tmissing_artifacts\trun_log\n" >"$VALIDATION_MATRIX_TSV"
  ...
  echo "BL-036 multi-run lane start: $DOC_TS"
  echo "runs=$RUNS"
  echo "mode=$RUN_MODE"
  echo "scenario=$SCENARIO_PATH"

  if ! command -v jq >/dev/null 2>&1; then
    printf "BL036-B1-001_contract_schema\tFAIL\tmissing_jq\t%s\n" "$SCENARIO_PATH" >>"$STATUS_TSV"
  ...
  if ! command -v shasum >/dev/null 2>&1; then
    printf "BL036-B1-001_contract_schema\tFAIL\tmissing_shasum\t%s\n" "$SCENARIO_PATH" >>"$STATUS_TSV"
  ...
  max_signature_divergence="$(jq -r '.bl036_contract_checks.replay_contract.max_signature_divergence // 0' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
  max_row_drift="$(jq -r '.bl036_contract_checks.replay_contract.max_row_drift // 0' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
  ...
  jq -r '.bl036_contract_checks.artifact_schema[]?' "$SCENARIO_PATH" >"$base_artifacts_tmp" 2>/dev/null || : >"$base_artifacts_tmp"
  if [[ "$RUN_MODE" == "execute_suite" ]]; then
    jq -r '.bl036_contract_checks.artifact_schema_execute_additions[]?' "$SCENARIO_PATH" >"$mode_artifacts_tmp" 2>/dev/null || : >"$mode_artifacts_tmp"
  else
    : >"$mode_artifacts_tmp"
  fi
  ...
  while [[ "$run_index" -le "$RUNS" ]]; do
    ...
    if [[ -f "$run_status_tsv" ]]; then
      ...
      contract_schema_result="$(awk -F'\t' '$1=="BL036-B1-001_contract_schema"{value=$2} END{if(value=="") value="MISSING"; print value}' "$run_status_tsv")"
      boundary_result="$(awk -F'\t' '$1=="BL036-B1-002_boundary_contract"{value=$2} END{if(value=="") value="MISSING"; print value}' "$run_status_tsv")"
      containment_result="$(awk -F'\t' '$1=="BL036-B1-003_containment_contract"{value=$2} END{if(value=="") value="MISSING"; print value}' "$run_status_tsv")"
    ...
    row_signature="$(hash_text "${lane_result}|${status_fail_count}|${contract_schema_result}|${boundary_result}|${containment_result}|${scenario_status}")"
  ...
  done < <(jq -r '.bl036_contract_checks.artifact_schema_multi_run[]?' "$SCENARIO_PATH" 2>/dev/null)

  if [[ "$signature_divergence_count" -le "$max_signature_divergence" ]]; then
    printf "BL036-B1-004_replay_hash_stability\tPASS\tsignature_divergence=%s max=%s row_drift=%s max_row_drift=%s\t%s\n" \
      "$signature_divergence_count" "$max_signature_divergence" "$row_drift_count" "$max_row_drift" "$REPLAY_HASHES_TSV" >>"$STATUS_TSV"
  else
    printf "BL036-B1-004_replay_hash_stability\tFAIL\tsignature_divergence=%s max=%s row_drift=%s max_row_drift=%s\t%s\n" \
      "$signature_divergence_count" "$max_signature_divergence" "$row_drift_count" "$max_row_drift" "$REPLAY_HASHES_TSV" >>"$STATUS_TSV"
  fi

  if [[ "$row_drift_count" -le "$max_row_drift" ]]; then
    printf "BL036-B1-006_hash_input_contract\tPASS\trow_drift=%s max=%s\t%s\n" \
      "$row_drift_count" "$max_row_drift" "$REPLAY_HASHES_TSV" >>"$STATUS_TSV"
  else
    printf "BL036-B1-006_hash_input_contract\tFAIL\trow_drift=%s max=%s\t%s\n" \
      "$row_drift_count" "$max_row_drift" "$REPLAY_HASHES_TSV" >>"$STATUS_TSV"
  fi

  total_artifact_failures=$((missing_result_artifact_count + aggregate_artifact_missing))
  if [[ "$total_artifact_failures" -eq 0 ]]; then
    printf "BL036-B1-005_artifact_schema_complete\tPASS\trun_and_aggregate_artifacts_present\t%s\n" "$VALIDATION_MATRIX_TSV" >>"$STATUS_TSV"
  else
    printf "BL036-B1-005_artifact_schema_complete\tFAIL\tartifact_failures=%s\t%s\n" "$total_artifact_failures" "$VALIDATION_MATRIX_TSV" >>"$STATUS_TSV"
  fi

  if [[ "$RUN_MODE" == "execute_suite" ]]; then
    printf "BL036-B1-007_execution_mode_contract\tPASS\tmode=execute_suite\t%s\n" "$STATUS_TSV" >>"$STATUS_TSV"
  else
    printf "BL036-B1-007_execution_mode_contract\tPASS\tmode=contract_only\t%s\n" "$STATUS_TSV" >>"$STATUS_TSV"
  fi
  ...
  if [[ "$summary_result" == "PASS" ]]; then
    printf "BL036-B1-009_soak_summary\tPASS\tresult=%s runs=%s\t%s\n" "$summary_result" "$RUNS" "$SOAK_SUMMARY_TSV" >>"$STATUS_TSV"
  else
    printf "BL036-B1-009_soak_summary\tFAIL\tresult=%s runs=%s\t%s\n" "$summary_result" "$RUNS" "$SOAK_SUMMARY_TSV" >>"$STATUS_TSV"
  fi
...
echo "BL-036 single-run lane start: $DOC_TS"
echo "mode=$RUN_MODE"
...
if [[ ! -f "$SCENARIO_PATH" ]]; then
  log_status "BL036-B1-001_contract_schema" "FAIL" "missing_scenario=$SCENARIO_PATH" "$SCENARIO_PATH"
...
if [[ "$failure_count" -eq 0 ]]; then
  if jq empty "$SCENARIO_PATH" >/dev/null 2>&1; then
    log_status "BL036-B1-001_contract_schema" "PASS" "scenario_json_parseable" "$SCENARIO_PATH"
  else
    log_status "BL036-B1-001_contract_schema" "FAIL" "scenario_json_not_parseable" "$SCENARIO_PATH"
    record_failure deterministic_contract_failure
  fi
fi
...
if [[ "$failure_count" -eq 0 ]]; then
  ...
  done < <(jq -r '.bl036_contract_checks.acceptance_ids[]?.id // empty' "$SCENARIO_PATH")
  if [[ "${#ACCEPTANCE_IDS[@]}" -ge 8 ]]; then
    log_status "BL036-B1-001_contract_schema" "PASS" "acceptance_ids=${#ACCEPTANCE_IDS[@]}" "$SCENARIO_PATH"
  else
    log_status "BL036-B1-001_contract_schema" "FAIL" "expected_acceptance_ids>=8 actual=${#ACCEPTANCE_IDS[@]}" "$SCENARIO_PATH"
    record_failure deterministic_contract_failure
  fi
fi

boundary_count="$(jq -r '(.bl036_contract_checks.required_guarded_boundaries // []) | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
has_ingress="$(jq -r '[.bl036_contract_checks.required_guarded_boundaries[]? | select(. == "dsp_ingress")] | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
has_intermediate="$(jq -r '[.bl036_contract_checks.required_guarded_boundaries[]? | select(. == "dsp_intermediate_accumulation")] | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
has_output="$(jq -r '[.bl036_contract_checks.required_guarded_boundaries[]? | select(. == "output_pre_host_write")] | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
if [[ "$boundary_count" -ge 3 && "$has_ingress" -ge 1 && "$has_intermediate" -ge 1 && "$has_output" -ge 1 ]]; then
  log_status "BL036-B1-002_boundary_contract" "PASS" "boundary_count=$boundary_count ingress=$has_ingress intermediate=$has_intermediate output=$has_output" "$SCENARIO_PATH"
else
  log_status "BL036-B1-002_boundary_contract" "FAIL" "boundary_count=$boundary_count ingress=$has_ingress intermediate=$has_intermediate output=$has_output" "$SCENARIO_PATH"
  record_failure deterministic_contract_failure
fi

containment_rule_count="$(jq -r '(.bl036_contract_checks.required_containment_rules // []) | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
has_nan_inf_rule="$(jq -r '[.bl036_contract_checks.required_containment_rules[]? | select(. == "nan_inf_replace_finite_fallback")] | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
has_denormal_rule="$(jq -r '[.bl036_contract_checks.required_containment_rules[]? | select(. == "denormal_flush_zero")] | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
has_post_limiter_rule="$(jq -r '[.bl036_contract_checks.required_containment_rules[]? | select(. == "post_limiter_non_finite_force_zero")] | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
has_hard_clamp_rule="$(jq -r '[.bl036_contract_checks.required_containment_rules[]? | select(. == "hard_clamp_pre_write")] | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
hard_clamp_max_abs="$(jq -r '.bl036_contract_checks.thresholds.hard_clamp_max_abs // 0' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
denormal_threshold="$(jq -r '.bl036_contract_checks.thresholds.denormal_abs_threshold_max // 0' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
if [[ "$containment_rule_count" -ge 4 && "$has_nan_inf_rule" -ge 1 && "$has_denormal_rule" -ge 1 && "$has_post_limiter_rule" -ge 1 && "$has_hard_clamp_rule" -ge 1 && "$hard_clamp_max_abs" != "0" && "$denormal_threshold" != "0" ]]; then
  log_status "BL036-B1-003_containment_contract" "PASS" "rules=$containment_rule_count hard_clamp_max_abs=$hard_clamp_max_abs denormal_threshold=$denormal_threshold" "$SCENARIO_PATH"
else
  log_status "BL036-B1-003_containment_contract" "FAIL" "rules=$containment_rule_count hard_clamp_max_abs=$hard_clamp_max_abs denormal_threshold=$denormal_threshold" "$SCENARIO_PATH"
  record_failure deterministic_contract_failure
fi
...
for class_name in "${required_taxonomy[@]}"; do
  count="$(jq -r --arg c "$class_name" '[.bl036_contract_checks.failure_taxonomy[]? | select(.failure_class == $c)] | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
  if [[ "$count" -eq 0 ]]; then
...
if [[ -z "$taxonomy_missing" ]]; then
  log_status "BL036-B1-008_failure_taxonomy_schema" "PASS" "required_classes_present" "$SCENARIO_PATH"
else
  log_status "BL036-B1-008_failure_taxonomy_schema" "FAIL" "missing_classes=${taxonomy_missing}" "$SCENARIO_PATH"
  record_failure deterministic_contract_failure
fi

default_mode="$(jq -r '.bl036_contract_checks.execution_contract.default_mode // empty' "$SCENARIO_PATH" 2>/dev/null || printf "")"
suite_mode="$(jq -r '.bl036_contract_checks.execution_contract.suite_mode // empty' "$SCENARIO_PATH" 2>/dev/null || printf "")"
if [[ "$default_mode" == "contract_only" && "$suite_mode" == "execute_suite" ]]; then
  log_status "BL036-B1-007_execution_mode_contract" "PASS" "default=$default_mode suite=$suite_mode" "$SCENARIO_PATH"
else
  log_status "BL036-B1-007_execution_mode_contract" "FAIL" "default=$default_mode suite=$suite_mode" "$SCENARIO_PATH"
  record_failure deterministic_contract_failure
fi

hash_include_count="$(jq -r '(.bl036_contract_checks.deterministic_hash_inputs.include // []) | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
hash_exclude_count="$(jq -r '(.bl036_contract_checks.deterministic_hash_inputs.exclude // []) | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
exclude_has_timestamp="$(jq -r '[.bl036_contract_checks.deterministic_hash_inputs.exclude[]? | select(. == "timestamp")] | length' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
if [[ "$hash_include_count" -ge 3 && "$hash_exclude_count" -ge 3 && "$exclude_has_timestamp" -ge 1 ]]; then
  log_status "BL036-B1-006_hash_input_contract" "PASS" "include=$hash_include_count exclude=$hash_exclude_count" "$SCENARIO_PATH"
else
  log_status "BL036-B1-006_hash_input_contract" "FAIL" "include=$hash_include_count exclude=$hash_exclude_count has_timestamp_exclusion=$exclude_has_timestamp" "$SCENARIO_PATH"
  record_failure deterministic_contract_failure
fi

THRESHOLD_SUITE_STATUS="$(jq -r '.bl036_contract_checks.thresholds.suite_status // "PASS"' "$SCENARIO_PATH" 2>/dev/null || printf "PASS")"
THRESHOLD_MAX_WARNINGS="$(jq -r '.bl036_contract_checks.thresholds.max_warnings // 0' "$SCENARIO_PATH" 2>/dev/null || printf "0")"
...

{
  ...
  printf "hard_clamp_max_abs=%s\n" "$hard_clamp_max_abs"
  printf "denormal_abs_threshold_max=%s\n" "$denormal_threshold"
  printf "hash_include_count=%s\n" "$hash_include_count"
  printf "hash_exclude_count=%s\n" "$hash_exclude_count"
  printf "required_guarded_boundaries=%s\n" "$boundary_count"
  printf "required_containment_rules=%s\n" "$containment_rule_count"
} >"$SCENARIO_CONTRACT_LOG"
...
done < <(jq -r '.bl036_contract_checks.artifact_schema[]?' "$SCENARIO_PATH")
if [[ "$RUN_MODE" == "execute_suite" ]]; then
  while IFS= read -r artifact_name; do
    [[ -n "$artifact_name" ]] || continue
    if [[ ! -f "$OUT_DIR/$artifact_name" ]]; then
  ...
  done < <(jq -r '.bl036_contract_checks.artifact_schema_execute_additions[]?' "$SCENARIO_PATH")
fi
if [[ "$artifact_missing_count" -eq 0 ]]; then
  log_status "BL036-B1-005_artifact_schema_complete" "PASS" "all_required_artifacts_present" "$OUT_DIR"
else
  log_status "BL036-B1-005_artifact_schema_complete" "FAIL" "missing_artifacts=$artifact_missing_list" "$OUT_DIR"
  record_failure missing_result_artifact
fi
...
if [[ "$lane_result" == "PASS" ]]; then
  log_status "BL036-B1-004_replay_hash_stability" "PASS" "single_run_signature_recorded" "$REPLAY_HASHES_TSV"
else
  log_status "BL036-B1-004_replay_hash_stability" "FAIL" "single_run_failures_present" "$REPLAY_HASHES_TSV"
fi
...
printf "run_01\t%s\t0\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t1\t%s\t%s\t1\tnone\t%s\n" \
  ...
  "$(awk -F'\t' '$1=="BL036-B1-001_contract_schema"{value=$2} END{if(value=="") value="MISSING"; print value}' "$STATUS_TSV")" \
  "$(awk -F'\t' '$1=="BL036-B1-002_boundary_contract"{value=$2} END{if(value=="") value="MISSING"; print value}' "$STATUS_TSV")" \
  "$(awk -F'\t' '$1=="BL036-B1-003_containment_contract"{value=$2} END{if(value=="") value="MISSING"; print value}' "$STATUS_TSV")" \
