...
RESULT_JSON="$OUT_DIR/locusq_production_p0_selftest_${TIMESTAMP}.json"
RUN_LOG="$OUT_DIR/locusq_production_p0_selftest_${TIMESTAMP}.run.log"
SELFTEST_TIMEOUT_SECONDS="${LOCUSQ_UI_SELFTEST_TIMEOUT_SECONDS:-75}"
if ! [[ "$SELFTEST_TIMEOUT_SECONDS" =~ ^[0-9]+$ ]]; then
...
echo "timeout_seconds=${SELFTEST_TIMEOUT_SECONDS}" | tee -a "$RUN_LOG"
echo "selftest_scope=${LOCUSQ_UI_SELFTEST_SCOPE:-}" | tee -a "$RUN_LOG"
...
if command -v jq >/dev/null 2>&1; then
  STATUS="$(jq -r '.payload.status // .status // .result.status // "unknown"' "$RESULT_JSON" 2>/dev/null || echo unknown)"
  OK="$(jq -r '.payload.ok // .ok // .result.ok // false' "$RESULT_JSON" 2>/dev/null || echo false)"
  echo "status=${STATUS}" | tee -a "$RUN_LOG"
  echo "ok=${OK}" | tee -a "$RUN_LOG"

  if [[ "$OK" != "true" ]]; then
    echo "failed_checks=" | tee -a "$RUN_LOG"
    jq -c '.result.checks // .payload.checks // []' "$RESULT_JSON" | tee -a "$RUN_LOG" || true
    osascript -e 'tell application "LocusQ" to quit' >/dev/null 2>&1 || true
    kill "$APP_PID" >/dev/null 2>&1 || true
    wait "$APP_PID" >/dev/null 2>&1 || true
    exit 1
