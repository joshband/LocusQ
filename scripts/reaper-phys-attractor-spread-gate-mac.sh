#!/usr/bin/env bash
# LocusQ attractor-spread host gate wrapper for headless REAPER.
#
# Opens a preloaded REAPER project with LocusQ on Track 1, runs the attractor-
# spread gate script, and exits 0 on pass / 1 on fail. Uses BlackHole 16ch so
# REAPER drives the real JUCE audio callback during the host acceptance run.
set -euo pipefail
...
GATE_SCRIPT="$ROOT_DIR/qa/reaper/reascripts/LocusQ_PhysicsAttractorSpread_Gate.lua"
...
RUN_DIR="$ROOT_DIR/TestEvidence/reaper_phys_attractor_spread_gate_${TIMESTAMP}"
mkdir -p "$RUN_DIR"
...
if [[ "$GATE_OVERALL" == "pass" ]]; then
  echo "PASS: Physics attractor-spread host gate"
  exit 0
fi

echo "FAIL: Physics attractor-spread host gate — see $GATE_STATUS" >&2
exit 1
