...
if [[ ! -x "$APP_EXEC" ]]; then
  ...
  echo "  scripts/standalone-ui-selftest-stage8-mac.sh /path/to/LocusQ.app"
  echo "  scripts/standalone-ui-selftest-stage8-mac.sh /path/to/LocusQ.app/Contents/MacOS/LocusQ"
  exit 2
fi
...
RESULT_JSON="$OUT_DIR/locusq_incremental_stage8_selftest_${TIMESTAMP}.json"
RUN_LOG="$OUT_DIR/locusq_incremental_stage8_selftest_${TIMESTAMP}.run.log"
...
echo "PASS: Stage 8 self-test completed." | tee -a "$RUN_LOG"
