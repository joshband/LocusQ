#!/usr/bin/env bash
# LocusQ attractor-crossing transient host gate wrapper for headless REAPER.
#
# Opens a preloaded REAPER project with LocusQ on Track 1, runs the attractor-
# crossing gate script, and exits 0 on pass / 1 on fail. Uses BlackHole 16ch
...
GATE_SCRIPT="$ROOT_DIR/qa/reaper/reascripts/LocusQ_PhysicsAttractorCrossing_Gate.lua"
...
RUN_DIR="$ROOT_DIR/TestEvidence/reaper_phys_attractor_crossing_gate_${TIMESTAMP}"
mkdir -p "$RUN_DIR"
...
if [[ "$GATE_OVERALL" == "pass" ]]; then
  echo "PASS: Physics attractor-crossing host gate"
  exit 0
fi

echo "FAIL: Physics attractor-crossing host gate — see $GATE_STATUS" >&2
exit 1
