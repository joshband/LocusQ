...
echo "primary_gate: standalone-ui-selftest-stage12-mac.sh"
echo "appium_enabled: ${UI_PR_GATE_WITH_APPIUM:-0}"
echo

if [[ "${UI_PR_GATE_WITH_SELFTEST:-1}" == "1" ]]; then
  SELFTEST_CMD=("$ROOT_DIR/scripts/standalone-ui-selftest-stage12-mac.sh")
  if [[ -n "$APP_PATH" ]]; then
    SELFTEST_CMD+=("$APP_PATH")
  fi

  if ! run_step "ui_stage12_selftest" "$OUT_DIR/ui_stage12_selftest.log" "${SELFTEST_CMD[@]}"; then
    echo
    echo "UI PR GATE RESULT: FAIL (stage12 self-test gate failed)"
    echo "status: $STATUS_TSV"
    exit 1
  fi
else
  echo -e "ui_stage12_selftest\tSKIP\t0\t-" | tee -a "$STATUS_TSV"
fi
