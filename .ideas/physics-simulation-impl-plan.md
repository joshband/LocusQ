...
- `scripts/reaper-phys-collision-transient-gate-mac.sh`: FAIL twice in real REAPER host runs after adding the new `phys_out_transient_N` DAW mirror (`gate_a_param_reg=true`, but all four scenarios stayed dark with `low_peak_transient=0.000`, `high_peak_transient=0.000`, `long_late_mean=0.000`). This is now the concrete host-level blocker for collision transient acceptance rather than an abstract “host lane missing” note.
...
| Collision gain transient as audible/DSP contract | `phys_collision_gain_scale`, `phys_collision_decay_ms` -> collision burst | `runtime-proven` | `locusq_physics_runtime_collision_transient_probe` proves gain-scale changes peak transient magnitude and decay-ms changes late-window persistence in the real processor path; `phys_out_transient_N` now exposes the same burst to DAW hosts | Resolve the current REAPER host-gate failure where the transient mirror stays dark across all four collision scenarios |
