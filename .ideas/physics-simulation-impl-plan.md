...
| Attractor-crossing gain spike | attractor crossing event -> gain transient | `runtime-proven` | `locusq_physics_runtime_attractor_crossing_probe` proves quiet baseline, true radius crossing, visible bridge transient, visible scene-published transient, and decay in the real processor path; `scripts/reaper-phys-attractor-crossing-gate-mac.sh` now proves the same burst contract on the DAW-visible transient mirror in REAPER | Add a second host or longer replay hardening if this becomes a promotion-grade completion claim |
| Collision gain transient as audible/DSP contract | `phys_collision_gain_scale`, `phys_collision_decay_ms` -> collision burst | `runtime-proven` | `locusq_physics_runtime_collision_transient_probe` proves gain-scale changes peak transient magnitude and decay-ms changes late-window persistence in the real processor path; `scripts/reaper-phys-collision-transient-gate-mac.sh` now proves the same burst contract in REAPER host playback with `gate_b_gain_scale=true` and `gate_c_decay=true` | Add replay hardening or a second host if this becomes a promotion-grade completion claim |
