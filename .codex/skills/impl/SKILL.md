...
Title: DSP Implementation Skill
Document Type: Skill
Author: APC Codex
Created Date: 2026-02-18
Last Modified Date: 2026-02-18
...
1. Regenerate implementation from `creative-brief`, `architecture`, `plan`, and `parameter-spec` inputs; avoid file-copy parity shortcuts.
2. Keep a one-to-one parameter coverage check: spec entry -> APVTS parameter -> DSP read path -> UI relay/attachment (if present).
3. Create `plugins/[Name]/Documentation/implementation-traceability.md` with clickable file references for each mapped parameter/control.
4. Treat WebView checklist failures as hard blockers, not warnings.
5. Capture build command(s) and resulting artifact paths in `plugins/[Name]/TestEvidence/build-summary.md`.
6. Validate implementation choices against `Documentation/invariants.md` and relevant `Documentation/adr/*.md` records.
7. If implementation changes a prior architectural decision, write or update an ADR before closing the phase.
8. Keep traceability docs cross-linked to `.ideas/architecture.md`, `.ideas/parameter-spec.md`, and ADR references.
9. Append validation snapshot/trend results to `plugins/[Name]/TestEvidence/validation-trend.md`.
...
**CRITICAL:** When implementing WebView plugins, you MUST verify ALL 8 points below. Use templates from `.codex/templates/webview/` and run validation script.
...
See: `.codex/troubleshooting/resolutions/webview-member-order-crash.md`
...
**Copy templates from:** `.codex/templates/webview/`
...
2. Check templates in `.codex/templates/webview/`
