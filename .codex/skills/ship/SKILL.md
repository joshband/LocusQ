...
Title: SKILL: PACKAGING (Cross-Platform)
Document Type: Skill
Author: APC Codex
Created Date: 2026-02-18
Last Modified Date: 2026-02-18
...
1. macOS packaging defaults to universal binaries (`arm64;x86_64`) for host parity across Apple Silicon and Intel.
2. Validate format presence before packaging: `*.vst3`, `*.app` (Standalone), and `*.component` on macOS.
3. Generate `dist/[Name]-v[version]/BUILD_MANIFEST.md` with build commands, format paths, and `lipo -archs` output where applicable.
4. If a selected format is missing, fail fast with explicit rebuild instructions.
5. Only mark shipping complete after archive creation and manifest verification.
