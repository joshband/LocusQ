-- LocusQ attractor-spread host gate — headless REAPER acceptance test.
--
-- Proves, in a real host session, that:
--   A  param_registration       required params are visible on one LocusQ instance
--   B  quiet_baseline          spread mirror stays quiet with attractor inactive
--   C  attractor_visible       attractor activation produces visible spread modulation
--   D  decay_observed          spread falls back down after attractor deactivation
--
...
local function log(msg)
  ...
  if NONINTERACTIVE then
    reaper.ShowConsoleMsg("[LQ-attractor-spread-gate] " .. msg .. "\n")
  end
end
...
local function fatal(msg, code)
  ...
  write_status({
    ...
    gate_c_attractor_visible = false,
...
local function configure_scene(ctx)
  ...
  set_linear(ctx, "phys_drag", 1.0, 0.0, 10.0)
  ...
  set_linear(ctx, "attractor_0_pos_x", 2.0, -25.0, 25.0)
  set_linear(ctx, "attractor_0_pos_y", 1.2, 0.0, 10.0)
  set_linear(ctx, "attractor_0_pos_z", 0.0, -25.0, 25.0)
  set_linear(ctx, "attractor_0_strength", 80.0, -100.0, 100.0)
  set_choice3(ctx, "attractor_0_falloff", 1)
  set_linear(ctx, "attractor_0_radius", 10.0, 0.1, 20.0)
  set_bool(ctx, "attractor_0_orbit_stabilize", false)
  set_bool(ctx, "attractor_0_active", false)
end
...
local function finish()
  ...
  write_status({
    ...
    gate_c_attractor_visible = gate_c_pass,
...
local function tick()
  ...
  if state == "BASELINE" then
    ...
    metrics.baseline_peak = math.max(metrics.baseline_peak, spread)
    defer_count = defer_count + 1
    ...
    if gate_b_pass then
      log("Gate B PASS: baseline spread stayed quiet with attractor inactive")
    else
      log("Gate B FAIL: baseline spread was already active before attractor enable")
    end
    set_bool(context, "attractor_0_active", true)
    state = "ACTIVE"
    defer_count = 0
    reaper.defer(tick)
    return
  ...
  if state == "ACTIVE" then
    local spread = get_normalized(context, "phys_out_spread_mod_0")
    metrics.active_peak = math.max(metrics.active_peak, spread)
    defer_count = defer_count + 1
    ...
    if gate_c_pass then
      log("Gate C PASS: attractor activation produced visible spread modulation")
    else
      log("Gate C FAIL: spread stayed too low after attractor activation")
    end
    set_bool(context, "attractor_0_active", false)
    state = "OFF"
    defer_count = 0
    reaper.defer(tick)
    return
  ...
  if state == "OFF" then
    local spread = get_normalized(context, "phys_out_spread_mod_0")
    metrics.off_mean = metrics.off_mean + spread
    ...
    if gate_d_pass then
      log("Gate D PASS: spread decayed back down after attractor disable")
    else
      log("Gate D FAIL: spread stayed too hot after attractor disable")
    end
    finish()
    return
  end
...
log("LocusQ attractor-spread host gate starting")
