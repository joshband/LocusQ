(() => {
  ...
  async function runIncrementalStage7SelfTest() {
    const report = {
  ...
  function primeFromCurrentState() {
    ...
    updateSceneStatusBadge();
    renderCounters();
    writeDiagnostics({ message: "stage7 ready" });
  }
  ...
  if (selfTestRequested) {
    ...
    window.setTimeout(() => {
      void runIncrementalStage7SelfTest();
    }, 900);
  } else {
    window.__LQ_SELFTEST_RESULT__ = {
      requested: false,
  ...
  window.runIncrementalStage7SelfTest = runIncrementalStage7SelfTest;
