(() => {
  ...
  async function runIncrementalStage10SelfTest() {
    const report = {
  ...
  function primeFromCurrentState() {
    ...
    updateSceneStatusBadge();
    renderCounters();
    writeDiagnostics({ message: "stage10 ready" });
  ...
  if (selfTestRequested) {
    ...
    window.setTimeout(() => {
      void runIncrementalStage10SelfTest();
    }, 900);
  } else {
    window.__LQ_SELFTEST_RESULT__ = {
      requested: false,
  ...
  window.runIncrementalStage10SelfTest = runIncrementalStage10SelfTest;
