(() => {
  ...
  async function runIncrementalStage9SelfTest() {
    const report = {
  ...
  function primeFromCurrentState() {
    ...
    updateSceneStatusBadge();
    renderCounters();
    writeDiagnostics({ message: "stage9 ready" });
  ...
  if (selfTestRequested) {
    ...
    window.setTimeout(() => {
      void runIncrementalStage9SelfTest();
    }, 900);
  } else {
    window.__LQ_SELFTEST_RESULT__ = {
      requested: false,
  ...
  window.runIncrementalStage9SelfTest = runIncrementalStage9SelfTest;
