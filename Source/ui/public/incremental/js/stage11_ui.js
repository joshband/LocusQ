(() => {
  ...
  async function runIncrementalStage11SelfTest() {
    const report = {
  ...
  function primeFromCurrentState() {
    ...
    updateSceneStatusBadge();
    renderCounters();
    writeDiagnostics({ message: "stage11 ready" });
  ...
  if (selfTestRequested) {
    ...
    window.setTimeout(() => {
      void runIncrementalStage11SelfTest();
    }, 900);
  } else {
    window.__LQ_SELFTEST_RESULT__ = {
      requested: false,
  ...
  window.runIncrementalStage11SelfTest = runIncrementalStage11SelfTest;
